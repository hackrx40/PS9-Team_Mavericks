...
def checkStamp(filename):
    ...
    results = model.predict(image)
    if results[0][0] < 1:
        result = "Stamp Detected"
        return result
    else:
        return "Stamp not detected"
             
def checkHand(filename):
    model = tf.keras.models.load_model('./ml-handwriting/keras_model.h5')   
    image = cv2.imread(filename)
    image = cv2.resize(image, (224, 224))
    image = image.astype('float32') / 255.0
    image = tf.expand_dims(image, axis=0)
...
def runTests(data):
    ...
    socketio.emit('status', {
        "done":4,
        "response": checkHand(open("filename.txt", "r").read()),
    })
